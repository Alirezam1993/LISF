...
!   the incorporation of GPS vertical displacement observations using 
!   the ensemble kalman filter (EnKF) method, into a land surface model. 
!
...
!  GPS DA component is adapted from GRACE DA.
!  
! !REVISION HISTORY:
!   27 Feb 2005: Sujay Kumar; Initial Specification
!   2024: Alireza Moghaddasi; Modified for GPS data assimilation
! 
! !USES: 
  use ESMF
  use enksgps_types
...
!-----------------------------------------------------------------------------
  public :: enksgps_init  ! Initialization for EnKF 
  public :: enksgps_setup
  public :: enksgps_increments ! compute analysis increments
  public :: enksgps_update ! apply the analysis increments
  public :: enksgps_diagnostics ! write EnKF related diagnostics
  public :: enksgps_final ! Finalization for EnKF
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: enksgps_struc ! data structure containing EnKF diagnostics
!EOP

  type, public ::  enksgps_dec
     logical     :: fileOpen
  ...
  end type enksgps_dec
!EOP  

  type(enksgps_dec), allocatable :: enksgps_struc(:,:)

contains

!BOP
! 
! !ROUTINE: enksgps_init
! \label{enksgps_init}
!  
! !INTERFACE: 
  subroutine enksgps_init()
! !USES: 
...
!  GMAO EnKF method for GPS data assimilation. The method reads 
!  the runtime settings from the LIS configuration file. 
! 
!EOP
    allocate(enksgps_struc(LIS_rc%nnest, LIS_rc%ndas))
   
  end subroutine enksgps_init
!BOP
! 
! !ROUTINE: enksgps_setup
! \label{enksgps_setup}
!  
! !INTERFACE: 
  subroutine enksgps_setup(k)
! !USES: 
...
!EOP
    ...
    do n=1,LIS_rc%nnest
       ...
       enksgps_struc(n,k)%fileOpen = .false.
    enddo
    if(mod(1800.0,LIS_rc%ts).ne.0) then 
       write(LIS_logunit,*) '[ERR] Please make sure that 30 minutes is a '
       write(LIS_logunit,*) '[ERR] multiple of the LIS clock timestep in GPS-DA mode'
       call LIS_endrun()
    endif

    do n=1,LIS_rc%nnest
       call ESMF_StateGet(LIS_OBS_State(n,k),itemCount=Nobjs,rc=status)
       call LIS_verify(status, 'ESMF_StateGet failed in enksgps_Mod')
       
       call ESMF_AttributeGet(LIS_OBS_State(n,k),name="Number Of Observations",&
            value=N_obs_size,rc=status)
       call LIS_verify(status, 'ESMF_AttributeGet failed in enksgps_Mod')
       
       if(LIS_rc%winnov(k).eq.1) then
          allocate(enksgps_struc(n,k)%norm_innov(Nobjs*N_obs_size))
          allocate(enksgps_struc(n,k)%innov(Nobjs*N_obs_size))
          allocate(enksgps_struc(n,k)%forecast_var(Nobjs*N_obs_size))
          allocate(enksgps_struc(n,k)%k_gain(LIS_rc%npatch(n,LIS_rc%lsm_index), &
               LIS_rc%nstvars(k)))
       endif
    enddo
  end subroutine enksgps_setup

!BOP
! 
! !ROUTINE: enksgps_increments
! \label{enksgps_increments}
!  
! !INTERFACE: 
  subroutine enksgps_increments(n,k)
! !USES: 
...
! 
! !DESCRIPTION: 
!  This routine computes the analysis increments for EnksGPS. The state variable
...
!  \begin{description}
!   \item[generateObservations]\ref{generateObservations_enksgps} \newline
!    obtain the observations
!   \item[lsmdagetobspred](\ref{lsmdagetobspred}) \newline
!    obtain model's estimate of the observations
!   \item[getObsPert](\ref{getObsPert_enksgps}) \newline
!    obtain the observation perturbations
!   \item[generateObsparam](\ref{generateObsparam_enksgps}) \newline
...
!   \item[assemble\_obs\_cov](\ref{assemble_obs_cov_enksgps})
...
!    modeling point.
!   \item[enksgps\_analysis](\ref{enksgps_analysis}) \newline
!    apply the EnksGPS filter to obtain the prognostic variable
!    increments. 
!   \item[row\_variance](\ref{row_variance_enksgps}) \newline
...
!----------------------------------------------------------------------------
    ...
    if(data_status) then  
       write(LIS_logunit,*) '[INFO] Assimilating GPS observations using GMAO EnKSGPS'
...
!                endif
!             endif
             call enksgps_analysis(gid,N_state,N_selected_obs, N_ens, &
                  obs_da, & 
                  obspred_da, &
                  obspert_da, &
                  obs_cov, &
       ...
       if(LIS_rc%winnov(k).eq.1) then 
          do i=1,Nobs
             ...
             !  add diag (R)
                enksgps_struc(n,k)%forecast_var(i) = std_innov(1)
                std_innov = std_innov+(Observations(i)%std)**2          
                std_innov = sqrt(std_innov)
                enksgps_struc(n,k)%norm_innov(i) = innov/std_innov(1)
                enksgps_struc(n,k)%innov(i) = innov
             else
                enksgps_struc(n,k)%norm_innov(i) = LIS_rc%udef
                enksgps_struc(n,k)%innov(i) = LIS_rc%udef
                enksgps_struc(n,k)%forecast_var(i) = LIS_rc%udef
             endif
          enddo
       endif
!----------------------------------------------------------------------------
...
!----------------------------------------------------------------------------
  ...
  end subroutine enksgps_increments

!BOP
! 
! !ROUTINE: enksgps_update
! \label{enksgps_update}
!
! !INTERFACE: 
  subroutine enksgps_update(n,k)
! !USES: 
...
end subroutine enksgps_update


!BOP
! 
! !ROUTINE: enksgps_diagnostics
!  \label{enksgps_diagnostics}
! 
! !INTERFACE:
  subroutine enksgps_diagnostics(n,k)
! !USES:
...
!   \item[getLSMvarnames](\ref{getLSMvarnames_enksgps}) \newline
!    retrieve the names of the LSM prognostic variables
!   \item[pruneVarname](\ref{pruneVarname_enksgps}) \newline
...
!   \item[getLSMdata](\ref{getLSMData_enksgps}) \newline
...
! * Forecast variance
!--------------------------------------------------------------------------
       if(.not.enksgps_struc(n,k)%fileopen.and.LIS_masterproc) then
  ...
  end subroutine enksgps_diagnostics
    ...
    if(LIS_rc%winnov(k).eq.1) then 
       ...
       if(LIS_masterproc) then 
          ...
          call LIS_verify(nf90_def_dim(ftn,'east_west',LIS_rc%obs_gnc(k),&
               dimID(1)),'nf90_def_dim for east_west failed in enksgps_mod')
          call LIS_verify(nf90_def_dim(ftn,'north_south',LIS_rc%obs_gnr(k),&
               dimID(2)),'nf90_def_dim for north_south failed in enksgps_mod')

          call LIS_verify(nf90_put_att(ftn,&
               NF90_GLOBAL,"missing_value", LIS_rc%udef),&
               'nf90_put_att failed for missing_value in enksgps_mod')
...
!--------------------------------------------------------------------------
          ...
          call LIS_verify(nf90_def_var(ftn,varname,&
               ...
               'nf90_def_var failed for '//trim(varname)//' in enksgps_mod')
          
#if(defined USE_NETCDF4)
          call LIS_verify(nf90_def_var_deflate(ftn,&
               ninnov_Id,&
               shuffle, deflate, deflate_level),&
               'nf90_def_var_deflate failed for ninnov in enksgps_mod')      
#endif
          call LIS_verify(nf90_put_att(ftn,ninnov_Id,&
               "standard_name",standard_name),&
               'nf90_put_att failed for '//trim(standard_name)//' in enksgps_mod')
...
!--------------------------------------------------------------------------
          ...
          call LIS_verify(nf90_def_var_deflate(ftn,&
               innov_Id,&
               shuffle, deflate, deflate_level),&
               'nf90_def_var_deflate for innov failed in enksgps_mod')             
#endif
          call LIS_verify(nf90_put_att(ftn,innov_Id,&
               "standard_name",standard_name),&
               'nf90_put_att failed for innov in enksgps_mod')
          
       endif
       
       call LIS_writevar_innov(ftn,n, k, ninnov_id, &
            enksgps_struc(n,k)%norm_innov)
       call LIS_writevar_innov(ftn,n, k, innov_id, &
            enksgps_struc(n,k)%innov)

       if(LIS_masterproc) then 
          call LIS_verify(nf90_close(ftn),&
               'nf90_close failed in enksgps_mod')
       endif
       ...
       if(LIS_masterproc) then 
          ...
          if(LIS_rc%wopt.eq."1d gridspace") then 
             call LIS_verify(nf90_def_dim(ftn,'ngrid',&
                  ...
                  dimID(1)),'nf90_def_dim for ngrid failed in enksgps_mod')
          ...
          call LIS_verify(nf90_put_att(ftn,&
               NF90_GLOBAL,"missing_value", LIS_rc%udef),&
               'nf90_put_att for missing_value failed in enksgps_mod')
          ...
          do v = 1, LIS_rc%nstvars(k)
             ...
             if(LIS_rc%wopt.eq."1d gridspace") then           
                call LIS_verify(nf90_def_var(ftn,varname,&
                     ...
                     'nf90_def_var for ensspread failed in enksgps_mod')
             ...
             call LIS_verify(nf90_def_var_deflate(ftn,&
                  ensspread_Id(v),&
                  shuffle, deflate, deflate_level),&
                  'nf90_def_var_deflate for ensspread failed in enksgps_mod')             
#endif
             call LIS_verify(nf90_put_att(ftn,ensspread_Id(v),&
                  "standard_name",standard_name),&
                  'nf90_put_att for ensspread failed in enksgps_mod')
             call LIS_verify(nf90_enddef(ftn),&
                  'nf90_enddef failed in enksgps_mod')
          end do
          deallocate(lsm_state_objs)          
       endif
...
!EOP
    ...
    write(unit=temp,fmt='(A9)') 'enksgps/'
...
! \label{assemble_obs_cov_enksgps}
!
! !INTERFACE:
...
!BOP
! 
! !ROUTINE: enksgps_final
! \label{enksgps_final}
! 
! !INTERFACE: 
  subroutine enksgps_final
! 
! !DESCRIPTION: 
...
!
!EOP
    deallocate(enksgps_struc)
  end subroutine enksgps_final

!BOP
! 
! !ROUTINE: getLSMvarnames
! \label{getLSMvarnames_enksgps}
! 
! !INTERFACE: 
...
! \label{generateObservations_enksgps}
! 
! !INTERFACE: 
...
!EOP
    ...
    do i=1,Nobjs
       ...
       call ESMF_AttributeGet(pertField, "Perturbation Type",&
            pert_type, rc=status)
       call LIS_verify(status, &
            "Perturbation Type attribute not found in enksgps_Mod")
...
! \label{generateObsparam_enksgps}
!
! !INTERFACE: 
