...
!BOP
! !MODULE: enksgps_general
! 
! this file contains a collection of general Ensemble Kalman filter
! subroutines and compact support subroutines for GPS data assimilation
!
! !REVISION HISTORY: 
! reichle, 20 Apr 2001
...
! Alireza, 2024 - Modified for GPS data assimilation
!EOP
module enksgps_general
  ...
  public :: enksgps_analysis
  
contains
  
!BOP
! 
! !ROUTINE: enksgps_analysis
! \label{enksgps_analysis}
! 
! !INTERFACE:  
  subroutine enksgps_analysis( gid, &
       N_state, N_obs, N_ens, &
...
! !DESCRIPTION:
!   
! Perform Linear-System EnKF update for GPS vertical displacement data assimilation
! 
! This implements the 8-step Linear-System EnKF algorithm as described in 
! the GPS DA literature:
! 1. X' = X - X̄ (state perturbations)  
! 2. Y' = H*X' (map to deformation space via Green's function)
! 3. P_yy^f = Y'*Y'^T/(N_ens-1) (forecast covariance in deformation space)
! 4. W = P_yy^f + R (total error covariance - key: smaller m×m system)
! 5. Solve W*b = d (linear system solution)
! 6. w = Y'^T * b (back-projection weights from deformation to ensemble space)
! 7. ΔX = X'*w/(N_ens-1) (water mass increment)
! 8. X^a = X̄ + ΔX (analysis state)
!
! IMPORTANT:
...
! are present, Hadamard product is applied to P_yy^f and Y'^T*b
!EOP
...
!    print*, 'stateincr ', State_incr
    ...
    ! ============================================================================
    ! LINEAR-SYSTEM EnKF ALGORITHM FOR GPS DISPLACEMENT DATA ASSIMILATION
    ! ============================================================================
    
    ! STEP 1: Compute state perturbations X' = X - X̄ (water mass space)
    ...
    ! STEP 2: Y' = H*X' (map state perturbations to deformation space via Green's function)
    ! Note: Obs_pred = H*X has already been calculated by noahmp401_getgpsdisppred
    
    Obs_pred_bar = sum( Obs_pred, 2) / real(N_ens)
    
    do n_e=1,N_ens
       Obs_pred_prime(:,n_e) = Obs_pred(:,n_e) - Obs_pred_bar  ! This is Y'
    end do

    ! STEP 3: Forecast covariance in deformation space P_yy^f = Y'*Y'^T/(N_ens-1)
    ...
    ! STEP 4: Total error covariance W = P_yy^f + R (still in deformation space)
    Repr_matrix = Repr_matrix + Obs_cov

    ! STEP 5: Prepare to solve linear system W*b = d 
    ! LU decomposition of W for efficient solving
    call enksgps_ludcmp( Repr_Matrix, N_obs, indx ) 
    ...
    do n_e=1,N_ens
       ...
       ! STEP 5: Solve linear system W*b = d (where d = innovation with perturbations)
       call enksgps_lubksb( Repr_matrix, N_obs, indx, rhs )

       if (.not. apply_hadamard) then
          
          ! STEP 6: Back-projection weights w = Y'^T * b 
          ! (converts deformation space solution back to ensemble weights)
          weights = matmul( transpose(Obs_pred_prime), rhs)
          
          ! STEP 7: Water mass increment ΔX = X' * w / (N_ens-1)
...
!    print*, 'stateincr ',state_incr
  ...
  end subroutine enksgps_analysis
  

!BOP
! 
! !ROUTINE: hadamard_for_repr_matrix
! \label{hadamard_for_repr_matrix_enksgps}
! 
! !INTERFACE:  
  subroutine hadamard_for_repr_matrix( N_obs, Observations,             & 
       ...
       Repr_Matrix )
! !USES:    
    use enksgps_types                     ! for obs_type
...
!  \label{get_gaspari_cohn_enksgps}
! 
! !INTERFACE:   
